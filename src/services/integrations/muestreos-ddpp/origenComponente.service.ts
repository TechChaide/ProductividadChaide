import type { OrigenComponente } from "@/types/integrations/muestreos-ddpp";
...
export const origenComponenteService = {
  async getAll(): Promise<BodyListResponse<OrigenComponente>> {
  ...
  async getById(id: number | string): Promise<BodyResponse<OrigenComponente>> {
  ...
  async save(data: OrigenComponente): Promise<BodyResponse<OrigenComponente>> {
    const response = await fetchWithAuth(API_URL, {
      method: 'POST',
