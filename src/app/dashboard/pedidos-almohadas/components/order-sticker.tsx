...
export default function OrderSticker({ order, className = "" }: Props) {
    ...
    <Card className={`shadow-lg w-full h-full flex-1 border border-gray-300 ${className}`}>
      <CardContent className="p-4 flex flex-col items-center justify-center w-full h-full">
        ...
        <div className="w-full flex flex-col items-center">
          <Barcode value={order.material + '-' + order.orden} width={260} height={46} />
          <span className="text-[11px] font-mono  text-gray-500">{order.material}-{order.orden}</span>
        </div>
