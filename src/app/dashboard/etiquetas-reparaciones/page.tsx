...
import OrderStickerReparaciones from "./components/order-sticker";

export default function ImpresionReparacionesPage() {
  ...
  const verificarReimpresion = async (
    QR: string,
    orden: string,
    etiquetaData: EtiquetaPlastificado
  ): Promise<boolean> => {
    try {
      const logsResponse =
        await LogReimpresionesService.getLogImpresionesPorCodigoBarrasF(QR);

      if (logsResponse && Array.isArray(logsResponse.data)) {
        // Filtrar logs que corresponden exactamente a esta etiqueta
        const logsDeEstaEtiqueta = filtrarLogsDeEtiqueta(
          logsResponse.data,
          etiquetaData
        );
        ...
        console.log(
          `[LOGS] Total logs encontrados: ${logsResponse.data.length}, logs de esta etiqueta: ${impresionesExistentes}`
        );
  ...
  const handlePrintZPL = async (
    ...
    try {
      ...
      console.log(`[ETIQUETAS-REPARACIONES] Iniciando impresión - Impresora de red: ${printerIP || 'No disponible'}`);
      ...
      if (printerIP) {
        ...
        console.log(`[ETIQUETAS-REPARACIONES] Usando impresora de red: ${printerIP}`);
      }
      ...
      if (payload.success && printerIP) {
        console.log(`[ETIQUETAS-REPARACIONES] Impresión en red exitosa: ${payload.printer}`);
      ...
      console.log('[ETIQUETAS-REPARACIONES] Usando BrowserPrint para impresión local');
      ...
      toast({
        ...
        description:
          `Etiquetas enviadas a imprimir (${cantidadCopias} ${
            cantidadCopias === 1 ? "copia" : "copias"
          }): \n [ Producto: ` +
          nombreProducto +
          ", \n Dimensiones: " +
          largo +
      ...
      if (Objeto && user?.code) {
        try {
          ...
          logGuardado = true;
          console.log(
            `[LOG] Reimpresión registrada correctamente (${cantidadCopias} ${
              cantidadCopias === 1 ? "copia" : "copias"
            })`
          );
        } catch (logError: any) {
          ...
          if (indiceEtiqueta !== -1) {
            ...
            nuevasEtiquetas[indiceEtiqueta] = {
              ...
              message: impresionExitosa
                ? `Etiqueta impresa correctamente (${cantidadCopias} ${
                    cantidadCopias === 1 ? "copia" : "copias"
  ...
  return (
    <>
      <Card className="grid grid-cols-2">
        <CardContent>
          <OrderStickerReparaciones
            codigoBarras={"20013483-251030216631091591"}
            añosGarantia={5}
            tipo={"II"}
            clase={"C: Plaza y media"}
            dimensiones={{
              ancho: 160,
              largo: 200,
              alto: 30,
            }}
            producto={"Prueba de Producto"}
            empresa={"CHAIDE"}

            tapa={"Tapa de prueba"}
            banda={"Banda de prueba"}
            tela_tapa={"Tela de tapa de prueba"}
            aislante={"Aislante de prueba"}
            lamina_textil={"Lámina textil de prueba"}
            numero_resortes={"Número de resortes de prueba"}
            peso_maximo_individual={"Peso máximo individual de prueba"}

            espuma_banda={"Espuma banda de prueba"}
            espuma_tapa={"Espuma tapa de prueba"}
            R1={"R1 de prueba"}
            R2={"R2 de prueba"}
            R3={"R3 de prueba"}
            LOTE={"LOTE de prueba"}
                    ...
                    <OrderStickerColchonesSimple
                      ...
                      dimensiones={{
                        ancho: etiquetaParaPreview
                          ? parseInt(etiquetaParaPreview.dimensiones.ancho) ||
                            160
                          : 160,
                        largo: etiquetaParaPreview
                          ? parseInt(etiquetaParaPreview.dimensiones.largo) ||
                            200
                          : 200,
                      ...
                      empresa={
                        etiquetaParaPreview
                          ? etiquetaParaPreview.etiquetaCompleta.Etiqueta_CodigoAntiguo?.toString() ||
                            "CHAIDE"
                          : "CHAIDE"
                      }
                    />
          ...
          <div className="p-2 flex items-center justify-between">
            <div className="flex flex-col">
              <span className="text-lg font-semibold">
                Impresión de Etiquetas Reparaciones
              </span>
                  ...
                  if (e.key === "Enter" && ordenInput.trim() !== "") {
                    ...
                    try {
                      ...
                      if (
                        ...
                        const puedeImprimir = await verificarReimpresion(
                          ordenInput.trim(),
                          etiquetaData.NUM_CABECERA,
                          etiquetaData
                        );
      ...
      >
        <AlertDialogContent className="max-w-md">
          ...
          <AlertDialogFooter>
            ...
            >
              Imprimir {cantidadAImprimir}{" "}
              {cantidadAImprimir === 1 ? "copia" : "copias"}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
