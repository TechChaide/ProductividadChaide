...
>(
  (
    ...
    const Comp = asChild ? (Slot as any) : "href" in props ? "a" : "button"
    const content = (
      <Comp
        ref={ref}
...
>(({ className, ...props }, ref) => {
  return (
    <ul
      ...
      className={cn(
        "mx-3.5 flex min-w-0 translate-x-px flex-col gap-1 border-l border-sidebar-border/50 pl-2.5",
        "group-data-[open=false]/menu-item:hidden",
        "group-data-[collapsible=icon]:hidden",
        className
      )}
      {...props}
