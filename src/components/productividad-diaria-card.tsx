...
export function ProductividadDiariaCard({ codigoEmpleado, nombre }: Props) {
  ...
  const registrosParaDSH = useMemo(() => {
    return sortedData.map((row) => ({
      ...
      TotalTiempoSTD: Number((Number(row.UNIDADES_PROD) || 0) * (Number(row.TiempoSTD) || 0) / 60),
    }));
  ...
  const egoPorHora = useMemo(() => {
    ...
    sortedData.forEach((row) => {
      const bloqueHora = agruparPorHora(row.HORA ?? "00:00");
      const actual = mapa.get(bloqueHora) ?? {
        totalTiempoSTD: 0,
        totalUnidades: 0,
        totalDefectos: 0,
      ...
      const tiempoTotalDia = Number((Number(row.UNIDADES_PROD) || 0) * (Number(row.TiempoSTD) || 0) / 60);
      ...
      mapa.set(bloqueHora, {
        totalTiempoSTD: actual.totalTiempoSTD + tiempoTotalDia,
    ...
    return Array.from(mapa.entries())
      .map(([hora, valores]) => {
        const horasNetas = 1;
        const totalA = horasNetas * 0.13;
        const descuentoProrrateado = valores.totalHorasDescontar / 8;
        const horasTeoricasEB = Math.max(0, horasNetas - totalA - descuentoProrrateado);
        const productividad = horasTeoricasEB > 0 ? (valores.totalTiempoSTD / horasTeoricasEB) * 100 : 0;
        ...
        return {
          ...
          productividad,
          calidad,
          totalTiempoSTD: valores.totalTiempoSTD,
          totalHorasDescontar: valores.totalHorasDescontar,
  ...
  const detalleEgoPorHora = useMemo(() => {
    const mapa = new Map<string, Map<string, {
      codMaterial: string;
      nombreMaterial: string;
      unidades: number;
      tiempoSTD: number;
    ...
    sortedData.forEach((row) => {
      const bloqueHora = agruparPorHora(row.HORA ?? "00:00");
      const codMat = row.CodMaterial ?? "—";

      if (!mapa.has(bloqueHora)) {
        mapa.set(bloqueHora, new Map());
      }
      const horaMapa = mapa.get(bloqueHora)!;
      const existing = horaMapa.get(codMat) ?? {
        codMaterial: codMat,
        nombreMaterial: row.NombreMaterial ?? codMat,
        unidades: 0,
        tiempoSTD: 0,
      };
      existing.unidades += Number(row.UNIDADES_PROD) || 0;
      existing.tiempoSTD += Number(row.TiempoSTD) || 0;
      horaMapa.set(codMat, existing);
    });

    return Array.from(mapa.entries())
      .map(([hora, materialesMap]) => ({
        hora,
        detalles: Array.from(materialesMap.values()).sort((a, b) => b.unidades - a.unidades),
      }))
      .sort((a, b) => horaAMinutos(a.hora) - horaAMinutos(b.hora));
  }, [sortedData]);

  const combinedHoraData = useMemo(() => {
    const mapa = new Map<string, { hora: string; unidades: number; ego: number }>();

    unidadesTotalesPorHora.forEach((item) => {
      mapa.set(item.hora, { hora: item.hora, unidades: item.unidades, ego: 0 });
    });

    egoPorHora.forEach((item) => {
      const existing = mapa.get(item.hora);
      mapa.set(item.hora, {
        hora: item.hora,
    ...
    return Array.from(mapa.values()).sort((a, b) => horaAMinutos(a.hora) - horaAMinutos(b.hora));
  }, [unidadesTotalesPorHora, egoPorHora]);

  const egoAcumulado = useMemo(() => {
    if (egoPorHora.length === 0) return resumenDsh.ego;

    const totalUnidades = egoPorHora.reduce((sum, h) => sum + h.totalUnidades, 0);
    const totalDefectos = egoPorHora.reduce((sum, h) => sum + h.totalDefectos, 0);
    const totalTiempoSTD = egoPorHora.reduce((sum, h) => sum + h.totalTiempoSTD, 0);
    const totalHorasDescontar = egoPorHora.reduce((sum, h) => sum + h.totalHorasDescontar, 0);

    const horasNetas = egoPorHora.length;
    const totalA = horasNetas * 0.13;
    const descuentoProrrateado = totalHorasDescontar / 8;
    const horasTeoricasEB = Math.max(0, horasNetas - totalA - descuentoProrrateado);

    const productividad = horasTeoricasEB > 0 ? (totalTiempoSTD / horasTeoricasEB) * 100 : 0;
    const calidad = totalUnidades > 0 ? (1 - totalDefectos / totalUnidades) * 100 : 0;

    return (productividad * calidad) / 100;
  }, [egoPorHora, resumenDsh]);

  const totalTiempoSTD = useMemo(() => {
    return egoPorHora.reduce((sum, h) => sum + h.totalTiempoSTD, 0);
  }, [egoPorHora]);
          ...
          <>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-4">
                  ...
                  <KpiTile
                    ...
                    label="Total Tiempo Estandar"
                    value={formatearNumero(totalTiempoSTD, 2)}
                    sub="Horas STD acumuladas"
                    color="yellow"
                  />
                  <KpiTile
                    icon={<Activity className="h-4 w-4" />}
                    label="EGO actual"
                    value={formatearNumero(egoAcumulado, 2)}
                    sub="EGO acumulado del día"
                    color="green"
                  />
                </div>
            ...
            <div className="rounded-xl border border-border bg-white p-4 lg:col-span-2">
              <div className="mb-3 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-sm font-semibold text-foreground">
                    EGO y unidades producidas por hora
                  </p>
                </div>
                <Badge variant="outline" className="text-xs">
                  EGO + Unidades
              ...
              <p className="mb-3 text-[11px] text-muted-foreground">
                Gráfico combinado de EGO y unidades por hora. La línea muestra EGO y las barras muestran unidades.
              </p>
              <ResponsiveContainer width="100%" height={320}>
                <ComposedChart
                  data={combinedHoraData}
                ...
                >
                  ...
                  <YAxis
                    ...
                    fontSize={11}
                    allowDecimals={false}
                    label={{ value: "Unidades", angle: -90, position: "insideLeft", offset: 8 }}
                  />
                  <YAxis
                    yAxisId="right"
                    orientation="right"
                    fontSize={11}
                    tickFormatter={(v) => `${formatearNumero(Number(v), 2)}%`}
                    label={{ value: "EGO %", angle: 90, position: "insideRight", offset: 8 }}
                  />
                  <Tooltip
                    formatter={(value: number, name: string) => {
                      if (name === "EGO") {
                        return [`${formatearNumero(Number(value), 2)}%`, name];
                      }
                      return [formatearNumero(Number(value)), name];
                    }}
                  />
                  <ReferenceLine x="07:00" yAxisId="left" stroke="#0f172a" strokeDasharray="4 4" />
                  <ReferenceLine x="08:00" yAxisId="left" stroke="#0f172a" strokeDasharray="4 4" />
                  <Bar
                    yAxisId="left"
                    dataKey="unidades"
                    name="Unidades"
            ...
            <div className="rounded-xl border border-border bg-white p-4 lg:col-span-2">
              ...
              <Accordion type="multiple" className="w-full">
                {egoPorHora.map((item) => {
                  const detalle = detalleEgoPorHora.find((d) => d.hora === item.hora);
                  return (
                    <AccordionItem key={item.hora} value={item.hora}>
                      <AccordionTrigger className="px-3 hover:no-underline">
                        <div className="flex items-center gap-4 text-[11px]">
                          <span className="font-bold text-slate-900 w-12">{item.hora}</span>
                          <span className="text-slate-600">
                            <span className="text-slate-500">Unidades:</span> {formatearNumero(item.totalUnidades)}
                          </span>
                          <span className="text-slate-600">
                            <span className="text-slate-500">Productividad:</span> {formatearNumero(item.productividad, 2)}%
                          </span>
                          <span className="text-slate-600">
                            <span className="text-slate-500">Calidad:</span> {formatearNumero(item.calidad, 2)}%
                          </span>
                          <span className="font-bold text-green-600">
                            EGO: {formatearNumero(item.ego, 2)}%
                          </span>
                        </div>
                      </AccordionTrigger>
                      <AccordionContent>
                        <div className="px-4 pb-2">
                          <table className="min-w-full divide-y divide-border text-[11px] text-left">
