...
export default function SelectedOrderDisplayAlmohadas({
  order,
  ...
  const handleStartSession = async () => {
    ...
    try {
      for (const currentUser of allUsers) {
        ...
        let fs = await authService.loginColaborador(currentUser.code);

        let dep = fs.user?.ficha?.DEPARTAMENTO || "";

        if (dep.toUpperCase().includes("ALMOHADAS") || dep.toUpperCase().includes("TECNOLOGIA")) {
          // Skip deactivation for Almohadas and Tecnologia collaborators
          continue;
        }
