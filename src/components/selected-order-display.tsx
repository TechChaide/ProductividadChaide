...
export default function SelectedOrderDisplay({ order }: SelectedOrderDisplayProps) {
  const { user, collaborators, areaProcessControls, activeSessions, fetchActiveSessions, estaciones, finishSession } = useUser();
  ...
  const areaControlForOrder = areaProcessControls.find(a => a.resp_ctrl_prod.trim() === order?.resp_ctrl_prod?.trim());

  const handleStartSession = async () => {
    if (!order || !user || !userStation) {
      toast({ title: "Error", description: "Falta información de Orden, Usuario o Estación para iniciar.", variant: "destructive" });
      return;
    }
    
    const areaControl = areaProcessControls.find(a => a.resp_ctrl_prod.trim() === order.resp_ctrl_prod.trim());
    if (!areaControl) {
      toast({ title: "Error de Configuración", description: `No se encontró el Area Process Control para "${order.resp_ctrl_prod}".`, variant: "destructive" });
      return;
    }
    ...
    try {
        for (const currentUser of allUsers) {
            ...
            const activeUserSessions = (sessionsResponse.data || []).filter(s => s.estado === 'A' && s.tipo_evento === 'beg');
        ...
        toast({ title: "Error de Limpieza", description: errorMessage, variant: "destructive" });
    ...
    try {
        ...
        toast({ title: "Éxito", description: `Sesión iniciada para ${allUsers.length} persona(s) en la estación ${userStation.nombre_estacion}.` });
    } catch (error) {
        const errorMessage = error instanceof Error ? error.message : 'No se pudo iniciar la sesión para todos los usuarios.';
  ...
  const handleFinishSession = async () => {
    if (!activeSessionOnThisStation) {
        toast({ title: "Error", description: "No hay una sesión activa en esta estación para finalizar.", variant: "destructive" });
        return;
    }
    await finishSession(); 
  };
  ...
  return (
    <Card className="shadow-lg w-full transition-all duration-300 ease-in-out flex-1">
      <CardHeader>
        <div className="flex justify-between items-start">
            <div>
                ...
                <CardDescription>Orden #{order.orden} - Estación de Orden: {order.maquina || 'N/A'}</CardDescription>
            </div>
      ...
      <CardContent className="space-y-3 text-sm">
        <div>
             <h3 className="text-xs font-medium text-muted-foreground">Descripción del Material</h3>
             <p className="font-medium">{order.descripcionMaterial}</p>
        </div>
        <Separator />
        <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              <h3 className="text-xs font-medium text-muted-foreground">Programada</h3>
              <p className="text-lg font-bold">{formatNumber(order.cantProgramada)}</p>
            </div>
            <div>
              <h3 className="text-xs font-medium text-muted-foreground">Notificada</h3>
              <p className="text-lg font-bold">{formatNumber(order.cantNotificada)}</p>
            </div>
            <div>
              <h3 className="text-xs font-medium text-muted-foreground">Pendiente</h3>
              <p className="text-lg font-bold text-destructive">{formatNumber(order.cantPendiente)}</p>
            </div>
        </div>
